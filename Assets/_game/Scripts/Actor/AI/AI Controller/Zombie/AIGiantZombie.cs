...
namespace Spicyy.AI
{
    public class AIGiantZombie : MobileAI, IEnemy
    {
        ...
        [Title("REGAIN HEALTH", titleAlignment: TitleAlignments.Centered)]
        [SerializeField] private float amountRegenHealthPerSecond; 
        ...
        private void OnEnable()
        {
            ...
            StartHealthRegenCoroutine();
            m_Health.OnDie += () => { Die(); aiState = AIState.Die; StopHealthRegenCoroutine(); };
        }
        ...
        #region Hành động
        private void StartHealthRegenCoroutine()
        {
        ...
        private void StopHealthRegenCoroutine()
        {
            if (healthRegenCoroutine != null)
            {
                StopCoroutine(healthRegenCoroutine);
        ...
        private IEnumerator HealthRegenCoroutine()
        {
            while (true)
            {
                yield return new WaitForSeconds(1f); // Đợi một giây
                if (m_Health.CurrentHealth < m_Health.MaxHealth)
                {
                    m_Health.Heal(amountRegenHealthPerSecond); // Hồi máu khi máu không đầy
                }
            }
        }
