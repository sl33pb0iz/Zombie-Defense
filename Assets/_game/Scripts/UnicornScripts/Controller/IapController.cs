...
private IEnumerator ContinuePromotionalPurchases()
{
    ...
    private void SetRewardPurchase(DataElementGift reward, string analytic = "iap")
    {
        switch (reward.type)
        {
            ...
            case TypeGift.SKIN:
            {
                ...
                PlayerDataManager.Instance.SetUnlockSkin(TypeEquipment.Skin, (int) Skin.SKIN_BADMON);
            }
                break;
            case TypeGift.PET:
            {
                PlayerDataManager.Instance.SetUnlockSkin(TypeEquipment.Pet, (int) TypePet.PET_5);
            }
                break;
            case TypeGift.REMOVE_ADS:
            {
                PlayerDataManager.Instance.SetNoAds();
    ...
    private void LogTracking(float gPrice, string gCurrency, Product product)
    {
        ...
        AppsFlyer.validateAndSendInAppPurchase(lincenseKey, googleReceipt.PayloadData.json,
            googleReceipt.PayloadData.signature, gPrice.ToString(), gCurrency, addparam, this);
...
{
    [SerializeField] private string priceDefault = "?";
